...
case class Progress(headerWriteCompleted: Boolean = false,
    contentWriteCompleted: Boolean = false,
    statusReceived: Boolean = false,
    headersReceived: Boolean = false,
    bodyPartReceived: Int = 0,
...
case class OnCompleted(progress: Progress, nanos: Long = nanoTime) extends HttpEvent
